const PROJECT_PREFIX = "roomy_project_";
...
router.post("/api/projects/update", async ({ request, user }) => {
  try {
    ...
    if (!id || !updates) return jsonError(400, "ID and updates required");

    const key = `${PROJECT_PREFIX}${id}`;

    // Puter's .update() method is perfect here for partial updates
    await userPuter.kv.update(key, updates);
